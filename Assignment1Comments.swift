import UIKit
/*  We create a structure rules in which we have a variable slot and a variable color, both represented by an int
    If we were to redo this program we would probably use a char or string for the color since using a int made
    it quite a challenge to remember each color and made reading the output harder. 
...
/* For a guess we used the same format of a struct as above. */
struct guess {
    var slot1: Int?
    var slot2: Int?
    var slot3: Int?
...
/*  Logic for guess 1, since we recived XO we know that if the first slot is White then either the third slot must be Orange or
...
/*  At this point we have 18 left, we use paper and pen logic (since writing down all the test cases would take forever and it is 
...
/*  This leaves us with 4 possible guesses, OBPB, OBPO, OBPP and OBPR.  We know that OBPR is not valid since we know that OBP is correct
